(ns sonarqube.main
  "Kotodama WASM entrypoint for the Sonarqube clean-room actor (L4) — Clojure port.
  ...
  facts (`sonarqube.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sonarqube")
(def tier "L4")
...
(def entity-specs
  [{:entity "Issue"       :plural "issues"       :id-prefix "sonarqub_iss"
    :fields [:key :severity :status :type :resolution :component :line :message] :required [:key :severity]
    :coerce {:line :int}                :refs {}}
   {:entity "Hotspot"     :plural "hotspots"     :id-prefix "sonarqub_hot"
    :fields [:key :status :message :component]                              :required [:key :status]
    :coerce {}                          :refs {}}
   {:entity "QualityGate" :plural "qualitygates" :id-prefix "sonarqub_qua"
    :fields [:status :ignoredConditions]                                    :required [:status :ignoredConditions]
    :coerce {:ignoredConditions :bool}  :refs {}}
   {:entity "Condition"   :plural "conditions"   :id-prefix "sonarqub_con"
    :fields [:status :metricKey :comparator :errorThreshold :actualValue]   :required [:status :metricKey]
    :coerce {}                          :refs {}}
   {:entity "Measure"     :plural "measures"     :id-prefix "sonarqub_mea"
    :fields [:metric :value :bestValue]                                     :required [:metric :value]
    :coerce {:bestValue :bool}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"sonarqube.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sonarqube-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sonarqube-compat/ready)
